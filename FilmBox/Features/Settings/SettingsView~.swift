...
struct SettingsView: View {
    ...
    private var storageSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "used: \(String(format: "%.1f", usedGB))gb / \(String(format: "%.0f", limitGB))gb"
                )
                ...
                GeometryReader { geometry in
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 4)
                            .fill(
                                percentage > 0.9
                                    ? Color.red.opacity(0.8) : Color.yellow.opacity(0.8)
                            )
    ...
    private func segmentButton(title: String, isSelected: Bool, action: @escaping () -> Void)
        -> some View
    {
    ...
    private func clearButton(
        title: String, comment: String, size: String, action: @escaping () -> Void
    ) -> some View {
        Button(action: action) {
            HStack {
